...
class MrpBom(models.Model):
    ...
    harga_bom = fields.Float(string='Harga BoM')
    ...
    bom_line_ids = fields.One2many(
        'mrp.bom.line', 'bom_id', 'BoM Lines', copy=True)
    ...
    sequence = fields.Integer(
        'Sequence', help="Gives the sequence order when displaying a list of bills of material.")
        ...
        for bom_line in res.bom_line_ids:
            if bom_line.operation_id:
                operation = res.operation_ids.filtered(
                    lambda op: op.name == bom_line.operation_id.name and op.workcenter_id == bom_line.operation_id.workcenter_id)
    ...
    def _bom_find_domain(self, product_tmpl=None, product=None, picking_type=None, company_id=False, bom_type=False):
        if product:
            ...
            domain = ['|', ('product_id', '=', product.id), '&', ('product_id',
                                                                  '=', False), ('product_tmpl_id', '=', product_tmpl.id)]
        elif product_tmpl:
        ...
        if company_id or self.env.context.get('company_id'):
            domain = domain + ['|', ('company_id', '=', False), ('company_id',
    ...
    def _bom_find(self, product_tmpl=None, product=None, picking_type=None, company_id=False, bom_type=False):
        ...
        domain = self._bom_find_domain(product_tmpl=product_tmpl, product=product,
                                       picking_type=picking_type, company_id=company_id, bom_type=bom_type)
        if domain is False:
            return self.env['mrp.bom']
    ...
    def _get_product2bom(self, products, bom_type=False, picking_type=False, company_id=False):
        ...
        domain = ['|', ('product_id', 'in', products.ids), '&', ('product_id',
                                                                 '=', False), ('product_tmpl_id', 'in', product_templates.ids)]
        if picking_type:
            domain += ['|', ('picking_type_id', '=', picking_type.id),
                       ('picking_type_id', '=', False)]
        if company_id or self.env.context.get('company_id'):
    ...
    def explode(self, product, quantity, picking_type=False):
        ...
        while bom_lines:
            ...
            if bom:
                ...
                bom_lines += [(line, current_line.product_id, converted_line_quantity,
                               current_line) for line in bom.bom_line_ids]
                for bom_line in bom.bom_line_ids:
    ...
    def _compute_attachments_count(self):
        for line in self:
            nbr_attach = self.env['mrp.document'].search_count([
                '|',
                '&', ('res_model', '=', 'product.product'), ('res_id',
                                                             '=', line.product_id.id),
    ...
    def _compute_allowed_operation_ids(self):
        for bom_line in self:
            if not bom_line.bom_id.operation_ids:
                ...
                bom_line.allowed_operation_ids = self.env['mrp.routing.workcenter'].search(
                    operation_domain)
    ...
    def _compute_allowed_operation_ids(self):
        for byproduct in self:
            if not byproduct.bom_id.operation_ids:
                ...
                byproduct.allowed_operation_ids = self.env['mrp.routing.workcenter'].search(
                    operation_domain)
