...
class ProductTemplate(models.Model):
    ...
    bom_line_ids = fields.One2many(
        'mrp.bom.line', 'product_tmpl_id', 'BoM Components')
    bom_ids = fields.One2many(
        'mrp.bom', 'product_tmpl_id', 'Bill of Materials')
    ...
    def _compute_mrp_product_qty(self):
        for template in self:
            template.mrp_product_qty = float_round(sum(template.mapped('product_variant_ids').mapped(
                'mrp_product_qty')), precision_rounding=template.uom_id.rounding)
...
class ProductProduct(models.Model):
    ...
    variant_bom_ids = fields.One2many(
        'mrp.bom', 'product_id', 'BOM Product Variants')
    bom_line_ids = fields.One2many(
        'mrp.bom.line', 'product_id', 'BoM Components')
    bom_count = fields.Integer('# Bill of Material',
                               compute='_compute_bom_count', compute_sudo=False)
    ...
    def _compute_mrp_product_qty(self):
        ...
        # TODO: state = done?
        domain = [('state', '=', 'done'), ('product_id', 'in',
                                           self.ids), ('date_planned_start', '>', date_from)]
        ...
        for product in self:
            if not product.id:
                ...
                continue
            product.mrp_product_qty = float_round(mapped_data.get(
    ...
    def _compute_quantities_dict(self, lot_id, owner_id, package_id, from_date=False, to_date=False):
        ...
        """
        bom_kits = self.env['mrp.bom']._get_product2bom(
            self, bom_type='phantom')
        kits = self.filtered(lambda p: bom_kits.get(p))
        res = super(ProductProduct, self - kits)._compute_quantities_dict(lot_id,
                                                                          owner_id, package_id, from_date=from_date, to_date=to_date)
    ...
    def action_open_quants(self):
        ...
        for product in self:
            bom = self.env['mrp.bom']._bom_find(
                product=product, bom_type='phantom')
            if bom:
