...
class ProductProduct(models.Model):
    ...
    def action_bom_cost(self):
        boms_to_recompute = self.env['mrp.bom'].search(['|', ('product_id', 'in', self.ids), '&', (
    ...
    def _compute_average_price(self, qty_invoiced, qty_to_invoice, stock_moves):
        ...
        bom = self.env['mrp.bom']._bom_find(
            product=self, company_id=stock_moves.company_id.id, bom_type='phantom')
        if not bom:
            return super()._compute_average_price(qty_invoiced, qty_to_invoice, stock_moves)
        value = 0
    ...
    def _compute_bom_price(self, bom, boms_to_recompute=False):
        ...
        return bom.product_uom_id._compute_price(bom.harga_bom + (total / bom.product_qty), self.uom_id)
